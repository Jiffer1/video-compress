function [DCencode_Y,DCencode_U,DCencode_V,...
    ...
    dcdict_Y,acdict_Y,...
    dcdict_U,acdict_U,...
    dcdict_V,acdict_V,motionvect,psnr]=PFrames_compress(targetFrame,anchorFrame)


[Y_target,U_target,V_target]=ycbcr_component420(targetFrame);
[Y_anchor,U_anchor,V_anchor]=ycbcr_component420(anchorFrame);

% 对P帧Y分量进行运动估计
[motionvect,Y_predict,psnr]=motionEstDS(Y_target,Y_anchor);
residue_Y=double(Y_target)-double(Y_predict);
%U分量和V分量直接相减
residue_U=double(U_target)-double(U_anchor);
residue_V=double(V_target)-double(V_anchor);
...
Y_dct=blockproc(residue_Y,[8,8],f1);
Y_quantization=blockproc(Y_dct,[8,8],f2);
U_dct=blockproc(residue_U,[8,8],f1);
U_quantization=blockproc(U_dct,[8,8],f3);
V_dct=blockproc(residue_V,[8,8],f1);
