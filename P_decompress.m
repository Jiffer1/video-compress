function frame=P_decompress(dcencode_Y,dcencode_U,dcencode_V,...
                           acencode_Y,acencode_U,acencode_V,...
                           dcdict_Y,acdict_Y,...
                           dcdict_U,acdict_U,...
                           dcdict_V,acdict_V,motionvect,anchorframe)
...
f1=@(block_struct) idct2(block_struct.data);
...
Y_comp=motionComp(Y_anchor,motionvect);
Y_rec=Y_420+double(Y_comp);
U_rec=U_420+double(U_anchor);
V_rec=V_420+double(V_anchor);

Y_rec=uint8(Y_rec);
U_rec=uint8(U_rec);
V_rec=uint8(V_rec);

frame=ycbcr2frame(Y_rec,U_rec,V_rec);
